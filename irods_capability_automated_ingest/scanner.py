...
class filesystem_scanner(scanner):
    ...
    def upload_file(self, hdlr_mod, logger, session, meta, op, exists, **options):
        ...
        if op == Operation.PUT_APPEND and exists:
            BUFFER_SIZE = 1024
            logger.info("appending object " + source_physical_fullpath + " from local filesystem, options = " + str(options))
            ...
            with open(source_physical_fullpath, "rb") as sfd:
                ...
                while True:
                    ...
                    tfd.write(buf)
            tfd.close()
            logger.info("succeeded", task="irods_FSappend_file", path=source_physical_fullpath)
        ...
        #If data object does exists, we know op=PUT_SYNC, and we re-copy whole file again, so it is fine also
            ...
            logger.info("uploading object " + source_physical_fullpath + " from local filesystem, options = " + str(options))
...
# --------------------------------------------------------------------------------------------------------------------------------------------------------
    def upload_file(self, hdlr_mod, logger, session, meta, op, exists, **options):
        ...
        if len(path_list) == 1:
            raise ValueError("Object name or bucket name missing from path to S3 object: [{}]".format(source_physical_fullpath))
        bucket_name = path_list[0]
        ...
        if total_bytes > MAXIMUM_SINGLE_THREADED_TRANSFER_SIZE and (not exists or op==Operation.PUT_SYNC):
            target_resc_name = options.get(kw.RESC_NAME_KW, '') or options.get(kw.DEST_RESC_NAME_KW, '')
            logger.info("Parallel put", task="irods_S3upload_file", path=source_physical_fullpath)
            bytecounts = self.parallel_upload_from_S3(logger, session, meta, bucket_name, object_name, dest_dataobj_logical_fullpath, total_bytes, target_resc_name, **options)
            if bytecounts!=total_bytes:
                raise RuntimeError("Parallel put from S3 failed: [{}]".format(source_physical_fullpath))
        ...
        #Single stream should happen for files <= 32MiB or for op=PUT_APPEND
            ...
            logger.info("Single stream put", task="irods_S3upload_file", path=source_physical_fullpath)
            self.single_stream_upload_from_S3(logger, session, meta, bucket_name, object_name, dest_dataobj_logical_fullpath, offset=tsize, **options)
    
    def parallel_upload_from_S3(self, logger, session, meta, bucket_name, object_name, dest, total_bytes, target_resc_name='', **options):
        """
        ...
        num_threads = max(1, min(multiprocessing.cpu_count(), RECOMMENDED_NUM_THREADS_PER_TRANSFER))
        open_options = {kw.NUM_THREADS_KW: str(num_threads), kw.DATA_SIZE_KW:str(total_bytes)}
        if target_resc_name:
            open_options[kw.RESC_NAME_KW] = target_resc_name
            open_options[kw.DEST_RESC_NAME_KW] = target_resc_name

        #initial open
        (Io, rawfile) = session.data_objects.open_with_FileRaw(dest, 'w', finalize_on_close=True, **open_options)
        ...
        chunk_ranges = [range(j * bytes_per_thread, (j+1)*bytes_per_thread) for j in range(num_threads-1)]
        ...
        for thread_id, byte_range in enumerate(chunk_ranges):
            if Io is None:
                Io = session.data_objects.open(dest, 'a', create=False, finalize_on_close=False, **ops)
            mgr.add_io(Io)
            futures.append(executor.submit(self.thread_copy_chunk_from_S3, Io, byte_range, bucket_name, object_name, mgr, str(thread_id)))
            Io = None
        bytecounts = sum([f.result() for f in futures])
        return bytecounts


    def thread_copy_chunk_from_S3(self, dst_data_object, chunk_range, bucket_name, object_name, dst_chunk_mgr, thread_debug_id):
        """
        ...
        response = self.client.get_object(bucket_name, object_name, offset=offset, length=length)
        bytecount = 0
    ...
    def single_stream_upload_from_S3(self, logger, session, meta, bucket_name, object_name, dest, offset=0, **options):
        """
        ...
        if len(etag)>32 and offset==0:
            logger.info("Multipart object", task="irods_S3upload_file", path="/"+bucket_name+"/"+object_name)
            ...
            if n_parts > 1 :
                ...
                logger.error("Incorrect multipart chunksize or error computing multipart checksum", task="irods_S3upload_file", path="/"+bucket_name+"/"+object_name)
        else:
            logger.info("Non-multipart object", task="irods_S3upload_file", path="/"+bucket_name+"/"+object_name)
            hash_md5 = hashlib.md5()
        ...
        if etag!=recreated_etag and offset==0:
            logger.error("checksums do not match! etag {} recreated etag {}".format(etag,recreated_etag), task="irods_S3upload_file", path="/"+bucket_name+"/"+object_name)
